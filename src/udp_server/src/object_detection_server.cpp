...
private:
    void onRGBFrame(cv::Mat &rgb, uint32_t timestamp)
    {
        ...
        std::cout << "Received RGB frame at timestamp: " << timestamp << std::endl;
    }
    ...
    void onDepthFrame(cv::Mat &depth, uint32_t timestamp)
    {
        ...
        std::cout << "Received Depth frame at timestamp: " << timestamp << std::endl;
    }
        ...
        if (!latest_rgb_.empty() && !latest_depth_.empty())
        {
            ...
            if (time_diff <= frame_sync_threshold_ms_)
            {
...

                {
                    ...
                    {
                        std::cerr << "Warning: Frame queue is full" << std::endl;
                    }
                }
            ...
            {
                std::cerr << "Warning: RGB and depth frames are out of sync (diff: "
                          << time_diff << " ms)" << std::endl;
            }
        }
        else
        {
            std::cerr << "Warning: Missing synchronized RGB or depth frame" << std::endl;
        }
    }
